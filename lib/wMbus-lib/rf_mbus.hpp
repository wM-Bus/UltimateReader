...
class RxLoop {
public:
  bool init(uint8_t mosi, uint8_t miso, uint8_t clk, uint8_t cs, uint8_t gdo0,
            uint8_t gdo2, float freq, bool syncMode);
  bool task();
...
private:
  ...
  SX1276 radio =
      new Module(RADIO_CS_PIN, RADIO_DIO0_PIN, RADIO_RST_PIN, RADIO_DIO1_PIN);
