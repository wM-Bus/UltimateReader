...
uint16_t crc16(uint8_t const t_message[], uint8_t t_nBytes,
               uint16_t t_polynomial, uint16_t t_init) {
...
bool crcValid(const uint8_t *t_bytes, uint8_t t_crcOffset) {
  ...
  uint16_t crcRead = (((uint16_t)t_bytes[t_crcOffset] << 8) |
                      t_bytes[t_crcOffset + 1]); // at the end
  if (crcCalc == crcRead) {
    // ESP_LOGV(TAG, "    calculated: 0x%04X, read: 0x%04X", crcCalc, crcRead);
    retVal = true;
  } else {
    // ESP_LOGV(TAG, "    calculated: 0x%04X, read: 0x%04X  !!!", crcCalc,
    // crcRead);
    retVal = false;
  }
  return retVal;
...
bool crcValid(const uint8_t *t_bytes, uint8_t t_crcOffset, uint8_t t_dataSize) {
  ...
  uint16_t crcRead =
      (((uint16_t)t_bytes[1] << 8) | t_bytes[0]); // at the begining
  if (crcCalc == crcRead) {
    // ESP_LOGV(TAG, "    calculated: 0x%04X, read: 0x%04X", crcCalc, crcRead);
    retVal = true;
  } else {
