...
bool mBusDecode(WMbusData &t_in, WMbusFrame &t_frame) {
  ...
  if (t_in.mode == 'C') {
    ...
    t_in.length -= 2;
    if (t_in.block == 'A') {
      // ESP_LOGVV(TAG, "Received C1 A frame");
      ...
      // ESP_LOGVV(TAG, "Received C1 B frame");
      // std::vector<unsigned char> frame(t_in.data, t_in.data + t_in.length);
      // std::string telegram = format_hex_pretty(frame);
      // telegram.erase(std::remove(telegram.begin(), telegram.end(), '.'),
      //                telegram.end());
    ...
    // ESP_LOGVV(TAG, "Received T1 A frame");
...
bool mBusDecodeFormatA(const WMbusData &t_in, WMbusFrame &t_frame) {
  ...
  uint8_t numDataBlocks =
      (L - 9 + 15) /
      16; // Data blocks are 16 bytes long + 2 CRC bytes (not counted in L)
  if ((L < 9) ||
      (((L - 9 + (numDataBlocks * 2))) >
       (t_in.length - BLOCK1A_SIZE))) { // add CRC bytes for each data block
    // ESP_LOGV(TAG, "Package (%u) too short for packet Length: %u",
    // t_in.length, L); ESP_LOGV(TAG, "  %u > %u", (L - 9 + (numDataBlocks *
    // 2)), (t_in.length - BLOCK1A_SIZE));
    return false;
  }
  ...
  for (uint8_t n{0}; n < numDataBlocks; ++n) {
    ...
    uint8_t blockSize =
        (MIN((L - 9 - (n * 16)),
             16)); // Maximum block size is 16 Data (without 2 CRC)
    ...
    // Get block data
    t_frame.frame.insert(
        (t_frame.frame.begin() + ((n * 16) + BLOCK1A_SIZE - 2)), blockStartPtr,
...
bool mBusDecodeFormatB(const WMbusData &t_in, WMbusFrame &t_frame) {
  ...
  if ((L < 12) ||
      ((L + 1) >
       t_in.length)) { // pod len mam miec zapisane ile bajtow odebralem
    // ESP_LOGV(TAG, "Package (%u) too short for packet Length: %u",
    // t_in.length, L); ESP_LOGV(TAG, "  %u > %u", (L + 1), t_in.length);
    return false;
  }
  ...
  t_frame.frame.insert(t_frame.frame.begin(), blockStartPtr,
                       (blockStartPtr + blockSize));
  t_frame.frame[0] -= 2;
  ...
  if (L > (L_OFFSET + 2)) {
    ...
    // Get Block3
    t_frame.frame.insert((t_frame.frame.end()), blockStartPtr,
                         (blockStartPtr + blockSize));
    t_frame.frame[0] -= 2;
  }
  return true;
