...
void xorit(unsigned char *srca, unsigned char *srcb, unsigned char *dest,
           int len) {
  for (int i = 0; i < len; ++i) {
...
bool decrypt_ELL_AES_CTR(std::vector<unsigned char> &frame,
  ...
  // en_bytes.erase(std::remove(en_bytes.begin(), en_bytes.end(), '.'),
  //                en_bytes.end());
  ...
  // dec_bytes.erase(std::remove(dec_bytes.begin(), dec_bytes.end(), '.'),
  //                 dec_bytes.end());
...
bool decrypt_TPL_AES_CBC_IV(std::vector<unsigned char> &frame,
  ...
  uint8_t tpl_num_encr_blocks =
      ((uint8_t)frame[13] >> 4) & 0x0f; // check if true for both short and long
  ...
  // dec_buffer.erase(std::remove(dec_buffer.begin(), dec_buffer.end(), '.'),
  //                  dec_buffer.end());
  ...
  // std::vector<unsigned char> dec_data(decrypted_data,
  //                                     decrypted_data + num_bytes_to_decrypt);
