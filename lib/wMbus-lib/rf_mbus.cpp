...
void IRAM_ATTR fifoGet(void) { dataInFifo = true; }
...
bool RxLoop::init(uint8_t mosi, uint8_t miso, uint8_t clk, uint8_t cs,
  ...
  Serial.println("Radio Starting to listen ...");
...
bool RxLoop::task() {
  do {
    bool reinit_needed =
        ((millis() - sync_time_) > max_wait_time_) ? true : false;
    if (reinit_needed) {
      rxLoop.state = INIT_RX;
    }
    ...
    case READ_DATA:
      if (dataInFifo) { // assert when Rx FIFO buffer threshold reached
        ...
        max_wait_time_ += extra_time_;
        receivedFlag =
            radio.fifoGet(data_in.data, rxLoop.length, &rxLoop.bytesRx);
    ...
    if (receivedFlag) {
      ...
      Serial.print("Have ");
      Serial.print(rxLoop.bytesRx);
      Serial.println(" bytes from radio Rx");
      if (rxLoop.length != data_in.length) {
        Serial.println("Length problem!");
        // ESP_LOGE(TAG, "Length problem: req(%d) != rx(%d)", rxLoop.length,
        //          data_in.length);
      }
      // if (this->syncMode) {
      //   Serial.println("Synchronus mode enabled.");
      // }
