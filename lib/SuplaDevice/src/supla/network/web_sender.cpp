...
void WebSender::send(int number, int precision) {
  ...
  snprintf(buf, sizeof(buf), "%.*f", printPrecission,
           static_cast<float>(number) / divider);
  send(buf);
}

void WebSender::sendName(const char *name) {
  char buf[100];
  snprintf(buf, sizeof(buf), " name=\"%s\" ", name ? name : "");
  send(buf);
}

void WebSender::sendId(const char *id) {
  char buf[100];
  snprintf(buf, sizeof(buf), " id=\"%s\" ", id ? id : "");
  send(buf);
}

void WebSender::sendNameAndId(const char *id) {
  char buf[100];
  snprintf(buf, sizeof(buf), " name=\"%s\" id=\"%s\" ", id ? id : "",
           id ? id : "");
  send(buf);
}
...
void WebSender::sendSelectItem(int value, const char *label, bool selected,
  ...
  char buf[100];
  if (emptyValue) {
    snprintf(buf, sizeof(buf), "<option value=\"\" %s>%s</option>",
