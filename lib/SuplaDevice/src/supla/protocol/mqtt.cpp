...
bool Supla::Protocol::Mqtt::isNetworkRestartRequested() { return false; }

uint32_t Supla::Protocol::Mqtt::getConnectionFailTime() { return 0; }

// By default MQTT protocol is enabled.
// It can be disabled if we have config class instance and when it is
// disabled there.
bool Supla::Protocol::Mqtt::isEnabled() { return enabled; }

bool Supla::Protocol::Mqtt::isConnectionError() { return error; }

bool Supla::Protocol::Mqtt::isConnecting() { return connecting; }
...
void Supla::Protocol::Mqtt::generateClientId(
  ...
  snprintf(
      result, MQTT_CLIENTID_MAX_SIZE, "SUPLA-%02X%02X%02X%02X%02X%02X%02X%02X",
      static_cast<unsigned char>(guid[0]), static_cast<unsigned char>(guid[1]),
      static_cast<unsigned char>(guid[2]), static_cast<unsigned char>(guid[3]),
      static_cast<unsigned char>(guid[4]), static_cast<unsigned char>(guid[5]),
      static_cast<unsigned char>(guid[6]), static_cast<unsigned char>(guid[7]));
}
...
void Supla::Protocol::Mqtt::onInit() {
  ...
  prefixLen = length - 1;
  if (prefix) {
    snprintf(prefix, length, "%s%s%s%s", customPrefix,
             customPrefixLength > 0 ? "/" : "", suplaTopic, hostname);
  ...
  SUPLA_LOG_INFO("Mqtt: getChannelCount (%d)", channelsCount);
}
...
void Supla::Protocol::Mqtt::publish(const char *topic, const char *payload,
                                    int qos, int retain, bool ignorePrefix) {
  if (prefix == nullptr) {
  ...
  if (verboseLog) {
    SUPLA_LOG_VERBOSE("MQTT publish(qos: %d, retain: %d): \"%s\" - \"%s\"", qos,
                      retainValue, mqttTopic.c_str(), payload);
  }
  publishImp(mqttTopic.c_str(), payload, qos, retainValue);
}

void Supla::Protocol::Mqtt::publishInt(const char *topic, int payload, int qos,
                                       int retain) {
  char buf[100] = {};
...
void Supla::Protocol::Mqtt::publishBool(const char *topic, bool payload,
                                        int qos, int retain) {
  char buf[6] = {};
...
// payload true -> on
// payload false -> off
void Supla::Protocol::Mqtt::publishOnOff(const char *topic, bool payload,
                                         int qos, int retain) {
  char buf[6] = {};
...
// payload true -> closed
// payload false -> open
void Supla::Protocol::Mqtt::publishOpenClosed(const char *topic, bool payload,
                                              int qos, int retain) {
  char buf[7] = {};
...
void Supla::Protocol::Mqtt::publishDouble(const char *topic, double payload,
...
void Mqtt::publishColor(const char *topic, uint8_t red, uint8_t green,
                        uint8_t blue, int qos, int retain) {
  char buf[12] = {};
...
void Supla::Protocol::Mqtt::publishChannelState(int channel) {
  SUPLA_LOG_INFO("Mqtt: publish channel %d state", channel);
  SUPLA_LOG_INFO("publishChannelState: getChannelCount (%d)", channelsCount);
  ...
  if (element == nullptr) {
    SUPLA_LOG_INFO("Mqtt: can't find element for channel %d", channel);
    return;
  }
  auto ch = element->getChannel();
  if (ch == nullptr) {
    SUPLA_LOG_INFO("Mqtt: failed to load channel object");
    return;
  }
  ...
  case SUPLA_CHANNELTYPE_RELAY: {
    ...
    case SUPLA_CHANNELFNC_CONTROLLINGTHEROLLERSHUTTER: {
      publishBool((topic / "is_calibrating").c_str(),
                  ch->getValueIsCalibrating(), -1, 1);
      publishInt((topic / "shut").c_str(), ch->getValueClosingPercentage(), -1,
                 1);
      break;
    }
  ...
  case SUPLA_CHANNELTYPE_THERMOMETER: {
    ...
    if (ch->getValueDouble() > -273) {
      publishDouble((topic / "temperature").c_str(), ch->getValueDouble(), -1,
                    1);
    }
    break;
  }
  case SUPLA_CHANNELTYPE_HUMIDITYANDTEMPSENSOR: {
    ...
    if (ch->getValueDoubleFirst() > -273) {
      publishDouble((topic / "temperature").c_str(), ch->getValueDoubleFirst(),
                    -1, 1);
    }
  ...
  case SUPLA_CHANNELTYPE_DIMMER: {
    ...
    publishInt((topic / "brightness").c_str(), ch->getValueBrightness(), -1, 1);
  ...
  case SUPLA_CHANNELTYPE_RGBLEDCONTROLLER: {
    ...
    publishInt((topic / "color_brightness").c_str(),
               ch->getValueColorBrightness(), -1, 1);
    publishBool((topic / "on").c_str(), ch->getValueColorBrightness() > 0, -1,
                1);
    publishColor((topic / "color").c_str(), ch->getValueRed(),
                 ch->getValueGreen(), ch->getValueBlue(), -1, 1);
    break;
  }
  ...
  case SUPLA_CHANNELTYPE_DIMMERANDRGBLED: {
    ...
    publishBool((topic / "rgb" / "on").c_str(),
                ch->getValueColorBrightness() > 0, -1, 1);
    publishBool((topic / "dimmer" / "on").c_str(), ch->getValueBrightness() > 0,
                -1, 1);
    publishColor((topic / "color").c_str(), ch->getValueRed(),
                 ch->getValueGreen(), ch->getValueBlue(), -1, 1);
    break;
    ...
    if (ch->getDefaultFunction() ==
      ...
      if (setpointHeat > INT16_MIN) {
        publishDouble((topic / "temperature_setpoint_heat").c_str(),
                      static_cast<double>(setpointHeat) / 100.0, -1, 1, 2);
      } else {
      ...
      if (setpointCool > INT16_MIN) {
        publishDouble((topic / "temperature_setpoint_cool").c_str(),
                      static_cast<double>(setpointCool) / 100.0, -1, 1, 2);
      } else {
      ...
      publishDouble((topic / "temperature_setpoint").c_str(),
                    static_cast<double>(tempreatureSetpoint) / 100.0, -1, 1, 2);
    }
    break;
  }
  ...
  case SUPLA_CHANNELTYPE_IMPULSE_COUNTER: {
    auto myTopic =
        MqttTopic("meters") / ch->meterId.c_str() / ch->meterType.c_str();
    // auto myTopic = topic / ch->meterType.c_str();
    // publishInt(myTopic.c_str(), ch->getValueInt64());
    // publishDouble(myTopic.c_str(), (double)(ch->getValueInt64() / 1000.0),
    // -1,
    //               -1, 3);
    publishDouble(myTopic.c_str(), (double)(ch->getValueInt64() / 1.0), -1, -1,
                  3);

    SUPLA_LOG_INFO("Mqtt: publish SUPLA_CHANNELTYPE_IMPULSE_COUNTER %.3f",
                   (double)ch->getValueInt64());
    break;
  }
...
void Supla::Protocol::Mqtt::publishExtendedChannelState(int channel) {
  ...
  case SUPLA_CHANNELTYPE_ELECTRICITY_METER: {
    ...
    if (ElectricityMeter::isFwdActEnergyUsed(extEMValue)) {
      publishDouble((topic / "total_forward_active_energy").c_str(),
                    ElectricityMeter::getTotalFwdActEnergy(extEMValue) /
                        100000.0,
                    -1, -1, 4);
    }
    ...
    if (ElectricityMeter::isRvrActEnergyUsed(extEMValue)) {
      publishDouble((topic / "total_reverse_active_energy").c_str(),
                    ElectricityMeter::getTotalRvrActEnergy(extEMValue) /
                        100000.0,
                    -1, -1, 4);
    }

    if (ElectricityMeter::isFwdBalancedActEnergyUsed(extEMValue)) {
      publishDouble((topic / "total_forward_balanced_active_energy").c_str(),
                    ElectricityMeter::getFwdBalancedActEnergy(extEMValue) /
                        100000.0,
                    -1, -1, 4);
    }

    if (ElectricityMeter::isRvrBalancedActEnergyUsed(extEMValue)) {
      publishDouble((topic / "total_reverse_balanced_active_energy").c_str(),
                    ElectricityMeter::getRvrBalancedActEnergy(extEMValue) /
                        100000.0,
                    -1, -1, 4);
    }

    if (ElectricityMeter::isVoltagePhaseAngle12Used(extEMValue)) {
      publishDouble((topic / "voltage_phase_angle_12").c_str(),
                    ElectricityMeter::getVoltagePhaseAngle12(extEMValue) / 10.0,
                    -1, -1, 1);
    }
    if (ElectricityMeter::isVoltagePhaseAngle13Used(extEMValue)) {
      publishDouble((topic / "voltage_phase_angle_13").c_str(),
    ...
    for (int phase = 0; phase < MAX_PHASES; phase++) {
      ...
      if (ElectricityMeter::isFwdActEnergyUsed(extEMValue)) {
        publishDouble((phaseTopic / "total_forward_active_energy").c_str(),
                      ElectricityMeter::getFwdActEnergy(extEMValue, phase) /
                          100000.0,
                      -1, -1, 4);
      }
      ...
      if (ElectricityMeter::isRvrActEnergyUsed(extEMValue)) {
        publishDouble((phaseTopic / "total_reverse_active_energy").c_str(),
                      ElectricityMeter::getRvrActEnergy(extEMValue, phase) /
                          100000.0,
                      -1, -1, 4);
      }
      ...
      if (ElectricityMeter::isFwdReactEnergyUsed(extEMValue)) {
        publishDouble((phaseTopic / "total_forward_reactive_energy").c_str(),
                      ElectricityMeter::getFwdReactEnergy(extEMValue, phase) /
                          100000.0,
                      -1, -1, 4);
      }
      ...
      if (ElectricityMeter::isRvrReactEnergyUsed(extEMValue)) {
        publishDouble((phaseTopic / "total_reverse_reactive_energy").c_str(),
                      ElectricityMeter::getRvrReactEnergy(extEMValue, phase) /
                          100000.0,
                      -1, -1, 4);
      }
  ...
  default:
    SUPLA_LOG_WARNING("Mqtt: channel type %d not supported for extended value",
                      ch->getChannelType());
    break;
  }
}
...
void Supla::Protocol::Mqtt::publishHADiscovery(int channel) {
  ...
  default:
    SUPLA_LOG_WARNING("Mqtt: publishHADiscovery: channel type %d not supported",
                      ch->getChannelType());
    break;
  }
}
...
void Mqtt::publishHADiscoveryBinarySensor(Supla::Element *element) {
  ...
  for (int i = 0; i < 2; i++) {
    bufferSize =
        snprintf(i ? payload : &c, i ? bufferSize : 1, cfg, prefix, prefix,
                 ch->getChannelNumber(), hostname,
                 ...
                 Supla::getBinarySensorChannelName(chFunction), objectId,
  ...
  for (int i = 0; i < 2; i++) {
    bufferSize =
        snprintf(i ? payload : &c, i ? bufferSize : 1, cfg, prefix, prefix,
                 ...
                 Supla::RegisterDevice::getSoftVer(),
                 element->getChannelNumber(),
                 Supla::getRelayChannelName(chFunction), objectId,
                 getDeviceClassStr(deviceClass)) +
        1;
  ...
  for (int i = 0; i < 2; i++) {
    bufferSize =
        snprintf(i ? payload : &c, i ? bufferSize : 1, cfg, prefix, prefix,
                 ...
                 objectId, static_cast<int>(sdc->getActivityTimeout())) +
        1;
...
Supla::Protocol::Mqtt::getHADiscoveryTopic(const char *type, char *objectId) {
  ...
  for (int i = 0; i < 2; i++) {
    bufferSize =
        snprintf(i ? payload : &c, i ? bufferSize : 1, cfg, prefix, prefix,
                 ...
                 Supla::RegisterDevice::getName(),
                 Supla::RegisterDevice::getSoftVer(),
                 element->getChannelNumber(), objectId,
  ...
  for (int actionIdx = 0; actionIdx < 8; actionIdx++) {
    ...
    if (enabled) {
      ...
      for (int i = 0; i < 2; i++) {
        bufferSize =
            snprintf(
                i ? payload : &c, i ? bufferSize : 1, cfg, hostname,
                getManufacturer(Supla::RegisterDevice::getManufacturerId()),
                Supla::RegisterDevice::getName(),
                Supla::RegisterDevice::getSoftVer(), prefix,
                ch->getChannelNumber(), atType, atType, buttonNumber, atType) +
            1;
  ...
  for (int i = 0; i < 2; i++) {
    bufferSize =
        snprintf(i ? payload : &c, i ? bufferSize : 1, cfg, prefix, prefix,
                 ...
                 Supla::RegisterDevice::getName(),
                 Supla::RegisterDevice::getSoftVer(),
                 element->getChannelNumber(), humanReadableParameterName,
                 objectId, units, phaseTopicPart, parameterName,
                 getStateClassStr(stateClass), getDeviceClassStr(deviceClass)) +
        1;
  ...
  for (int phase = 0; phase < MAX_PHASES; phase++) {
    ...
    if (ElectricityMeter::isFwdActEnergyUsed(extEMValue)) {
      publishHADiscoveryEMParameter(
          element, parameterId, "total_forward_active_energy", "kWh",
          Supla::Protocol::HAStateClass_TotalIncreasing,
          Supla::Protocol::HADeviceClass_Energy);
    }
    ...
    if (ElectricityMeter::isRvrActEnergyUsed(extEMValue)) {
      publishHADiscoveryEMParameter(
          element, parameterId, "total_reverse_active_energy", "kWh",
          Supla::Protocol::HAStateClass_TotalIncreasing,
          Supla::Protocol::HADeviceClass_Energy);
    }
    ...
    if (ElectricityMeter::isFwdReactEnergyUsed(extEMValue)) {
      publishHADiscoveryEMParameter(
          element, parameterId, "total_forward_reactive_energy", "kvarh",
          Supla::Protocol::HAStateClass_TotalIncreasing,
          Supla::Protocol::HADeviceClass_Energy);
    }
    ...
    if (ElectricityMeter::isRvrReactEnergyUsed(extEMValue)) {
      publishHADiscoveryEMParameter(
          element, parameterId, "total_reverse_reactive_energy", "kvarh",
          Supla::Protocol::HAStateClass_TotalIncreasing,
          Supla::Protocol::HADeviceClass_Energy);
    }
    ...
    parameterId++;
    if (ElectricityMeter::isFreqUsed(extEMValue)) {
      publishHADiscoveryEMParameter(element, parameterId, "frequency", "Hz",
    ...
    parameterId++;
    if (ElectricityMeter::isVoltageUsed(extEMValue)) {
      publishHADiscoveryEMParameter(element, parameterId, "voltage", "V",
                                    Supla::Protocol::HAStateClass_Measurement,
    ...
    parameterId++;
    if (ElectricityMeter::isCurrentUsed(extEMValue)) {
      publishHADiscoveryEMParameter(element, parameterId, "current", "A",
                                    Supla::Protocol::HAStateClass_Measurement,
    ...
    parameterId++;
    if (ElectricityMeter::isPowerActiveUsed(extEMValue)) {
      publishHADiscoveryEMParameter(element, parameterId, "power_active", "W",
                                    Supla::Protocol::HAStateClass_Measurement,
    ...
    if (ElectricityMeter::isPowerReactiveUsed(extEMValue)) {
      publishHADiscoveryEMParameter(
          element, parameterId, "power_reactive", "var",
          Supla::Protocol::HAStateClass_Measurement,
    ...
    if (ElectricityMeter::isPowerApparentUsed(extEMValue)) {
      publishHADiscoveryEMParameter(
          element, parameterId, "power_apparent", "VA",
          Supla::Protocol::HAStateClass_Measurement,
    ...
    parameterId++;
    if (ElectricityMeter::isPowerFactorUsed(extEMValue)) {
      publishHADiscoveryEMParameter(element, parameterId, "power_factor", "",
                                    Supla::Protocol::HAStateClass_Measurement,
    ...
    parameterId++;
    if (ElectricityMeter::isPhaseAngleUsed(extEMValue)) {
      publishHADiscoveryEMParameter(element, parameterId, "phase_angle", "°",
                                    Supla::Protocol::HAStateClass_Measurement,
...
bool Supla::Protocol::Mqtt::isRegisteredAndReady() { return connected; }
  ...
  for (int i = 0; i < 2; i++) {
    bufferSize =
        snprintf(i ? payload : &c, i ? bufferSize : 1, cfg, prefix, prefix,
                 ...
                 getManufacturer(Supla::RegisterDevice::getManufacturerId()),
                 Supla::RegisterDevice::getName(),
                 Supla::RegisterDevice::getSoftVer(), objectId,
                 subId == 1 ? "/rgb" : "", subId == 1 ? "/rgb" : "") +
        1;
  ...
  for (int i = 0; i < 2; i++) {
    bufferSize =
        snprintf(i ? payload : &c, i ? bufferSize : 1, cfg, prefix, prefix,
                 ...
                 Supla::RegisterDevice::getName(),
                 Supla::RegisterDevice::getSoftVer(), objectId,
                 subchannels ? "/dimmer" : "", subchannels ? "/dimmer" : "") +
        1;
...
void Mqtt::processRelayRequest(const char *part, const char *payload,
                               Supla::Element *element) {
...
void Mqtt::processRollerShutterRequest(const char *part, const char *payload,
                                       Supla::Element *element) {
  TSD_SuplaChannelNewValue newValue = {};
  element->fillSuplaChannelNewValue(&newValue);
...
void Mqtt::processRGBWRequest(const char *part, const char *payload,
                              Supla::Element *element) {
  TSD_SuplaChannelNewValue newValue = {};
  element->fillSuplaChannelNewValue(&newValue);
...
void Mqtt::processRGBRequest(const char *part, const char *payload,
                             Supla::Element *element) {
  TSD_SuplaChannelNewValue newValue = {};
  element->fillSuplaChannelNewValue(&newValue);
...
void Mqtt::processDimmerRequest(const char *part, const char *payload,
                                Supla::Element *element) {
  TSD_SuplaChannelNewValue newValue = {};
  element->fillSuplaChannelNewValue(&newValue);
...
void Mqtt::publishHADiscoveryHVAC(Supla::Element *element) {
  ...
  if (tempChannelNo != element->getChannelNumber()) {
    snprintf(temperatureTopic, sizeof(temperatureTopic),
             "%s/channels/%i/state/temperature", prefix, tempChannelNo);
    ...
    if (thermometerEl != nullptr &&
      ...
      snprintf(humidityTopic, sizeof(humidityTopic),
               "%s/channels/%i/state/humidity", prefix, tempChannelNo);
    }
  }
  ...
  for (int i = 0; i < 2; i++) {
    bufferSize =
        snprintf(
            i ? payload : &c, i ? bufferSize : 1, cfg, prefix, prefix,
            ch->getChannelNumber(), hostname,
            getManufacturer(Supla::RegisterDevice::getManufacturerId()),
            Supla::RegisterDevice::getName(),
            Supla::RegisterDevice::getSoftVer(), element->getChannelNumber(),
            objectId, temperatureTopic, humidityTopic,
...
void Mqtt::processHVACRequest(const char *topic, const char *payload,
                              Supla::Element *element) {
  TSD_SuplaChannelNewValue newValue = {};
  element->fillSuplaChannelNewValue(&newValue);
