...
public:
  ...
  void publish(const char *topic, const char *payload, int qos = -1,
               int retain = -1, bool ignorePrefix = false);
  void publishInt(const char *topic, int payload, int qos = -1,
                  int retain = -1);
  void publishBool(const char *topic, bool payload, int qos = -1,
                   int retain = -1);
  ...
  void publishDouble(const char *topic, double payload, int qos = -1,
                     int retain = -1, int precision = 2);
  void publishColor(const char *topic, uint8_t red, uint8_t green, uint8_t blue,
                    int qos = -1, int retain = -1);
  ...
  void
  sendExtendedChannelValueChanged(uint8_t channelNumber,
                                  TSuplaChannelExtendedValue *value) override;

  bool processData(const char *topic, const char *payload);
  void processRelayRequest(const char *topic, const char *payload,
                           Supla::Element *element);
  void processRGBWRequest(const char *topic, const char *payload,
                          Supla::Element *element);
  void processRGBRequest(const char *topic, const char *payload,
                         Supla::Element *element);
  void processDimmerRequest(const char *topic, const char *payload,
                            Supla::Element *element);
  void processHVACRequest(const char *topic, const char *payload,
                          Supla::Element *element);
  void processRollerShutterRequest(const char *topic, const char *payload,
                                   Supla::Element *element);
...
protected:
  ...
  void
  publishHADiscoveryEMParameter(Supla::Element *element, int parameterId,
                                const char *parameterName, const char *units,
  ...
  virtual void publishImp(const char *topic, const char *payload, int qos,
                          bool retain) = 0;
