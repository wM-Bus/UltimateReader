...
Storage *Storage::Instance() { return instance; }

Config *Storage::ConfigInstance() { return configInstance; }
...
bool Storage::RegisterSection(int sectionId, int offset, int size, bool addCrc,
                              bool addBackupCopy) {
  if (Instance()) {
...
Storage::Storage(unsigned int storageStartingOffset, unsigned int availableSize,
                 enum WearLevelingMode wearLevelingMode)
    : storageStartingOffset(storageStartingOffset),
      availableSize(availableSize), wearLevelingMode(wearLevelingMode) {
  instance = this;
}
...
void Storage::deleteAll() {
  char emptyTag[5] = {};
  writeStorage(storageStartingOffset, (unsigned char *)&emptyTag,
               sizeof(emptyTag));
  if (stateStorage != nullptr) {
...
int Storage::updateStorage(unsigned int offset, const unsigned char *buf,
                           int size) {
      ...
      for (int entry = 0; entry < (ptr->addBackupCopy ? 2 : 1); entry++) {
        ...
        if (ptr->addCrc) {
          ...
          readStorage(offset + size,
                      reinterpret_cast<unsigned char *>(&readCrc),
                      sizeof(readCrc));
          ...
          if (readCrc != calcCrc) {
            SUPLA_LOG_WARNING(
                "Storage: special section crc check failed %d != %d", readCrc,
                calcCrc);
        ...
        if (isDataDifferent) {
          ...
          if (ptr->addCrc) {
            ...
            writeStorage(offset + size,
                         reinterpret_cast<unsigned char *>(&calcCrc),
                         sizeof(calcCrc));
          }
        }
      }
...
void Storage::LoadStateStorage() {
  ...
  if (Instance()->stateStorage->prepareLoadState()) {
    if (Instance()->isAddChannelNumbersEnabled()) {
      ...
      SUPLA_LOG_INFO("Storage: channelsCount (%d)", channelsCount);
...
void Storage::enableChannelNumbers() { addChannelNumbers = true; }

bool Storage::isAddChannelNumbersEnabled() const { return addChannelNumbers; }
