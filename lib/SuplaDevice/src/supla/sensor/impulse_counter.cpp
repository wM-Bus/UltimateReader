...
ImpulseCounter::ImpulseCounter(Supla::Io *io, int _impulsePin,
                               bool _detectLowToHigh, bool _inputPullup,
                               unsigned int _debounceDelay)
    : ImpulseCounter(_impulsePin, _detectLowToHigh, _inputPullup,
                     _debounceDelay) {
  this->io = io;
}

ImpulseCounter::ImpulseCounter(int _impulsePin, bool _detectLowToHigh,
                               bool _inputPullup, unsigned int _debounceDelay)
    : impulsePin(_impulsePin), debounceDelay(_debounceDelay),
      detectLowToHigh(_detectLowToHigh), inputPullup(_inputPullup) {
  prevState = (detectLowToHigh == true ? LOW : HIGH);

  SUPLA_LOG_INFO("IC[%d]: impulsePin(%d), delay(%d ms)", getChannelNumber(),
                 impulsePin, debounceDelay);
  if (impulsePin < 0) {
