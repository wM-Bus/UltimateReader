...
Channel::Channel(int number) {
  ...
  SUPLA_LOG_INFO("addChannel: channel %d", number);
...
Channel *Channel::Begin() { return firstPtr; }
...
Channel *Channel::next() { return nextPtr; }
  ...
  if (setNewValue(newValue)) {
    ...
    if (isnan(dbl)) {
      ...
      SUPLA_LOG_DEBUG("Channel(%d) value changed to %d.%02d", channelNumber,
                      static_cast<int>(dbl),
                      abs(static_cast<int>(dbl * 100) % 100));
    }
  }
}
...
void Channel::setNewValue(double temp, double humi) {
  ...
  if (setNewValue(newValue)) {
    runAction(ON_CHANGE);
    runAction(ON_SECONDARY_CHANNEL_CHANGE);
    SUPLA_LOG_DEBUG("Channel(%d) value changed to temp(%.2f), humi(%.2f)",
                    channelNumber, temp, humi);
  }
}
...
int32_t Channel::getDefaultFunction() const { return defaultFunction; }

void Channel::setFlag(uint64_t flag) { channelFlags |= flag; }

void Channel::unsetFlag(uint64_t flag) { channelFlags &= ~flag; }

void Channel::setFuncList(_supla_int_t functions) {
  functionsBitmap = functions;
}

_supla_int_t Channel::getFuncList() const { return functionsBitmap; }
...
uint64_t Channel::getFlags() const { return channelFlags; }

void Channel::setActionTriggerCaps(_supla_int_t caps) { setFuncList(caps); }

_supla_int_t Channel::getActionTriggerCaps() { return getFuncList(); }

int Channel::getChannelNumber() const { return channelNumber; }
...
void Channel::clearSendValue() { changedFields &= ~CHANNEL_SEND_VALUE; }

void Channel::setSendGetConfig() { changedFields |= CHANNEL_SEND_GET_CONFIG; }
...
void Channel::setSendStateInfo() { changedFields |= CHANNEL_SEND_STATE_INFO; }
...
void Channel::sendUpdate() {
  if (isValueUpdateReady()) {
    ...
    for (auto proto = Supla::Protocol::ProtocolLayer::first(); proto != nullptr;
         proto = proto->next()) {
      proto->sendChannelValueChanged(channelNumber, value, state,
                                     validityTimeSec);
    }
...
TSuplaChannelExtendedValue *Channel::getExtValue() { return nullptr; }
...
bool Channel::isUpdateReady() const { return changedFields != 0; }

bool Channel::isExtended() const { return false; }
...
void Channel::setNewValue(uint8_t red, uint8_t green, uint8_t blue,
                          uint8_t colorBrightness, uint8_t brightness) {
  char newValue[SUPLA_CHANNELVALUE_SIZE];
  memset(newValue, 0, SUPLA_CHANNELVALUE_SIZE);
...
bool Channel::getValueBool() { return value[0] != 0; }

uint8_t Channel::getValueRed() { return value[4]; }

uint8_t Channel::getValueGreen() { return value[3]; }

uint8_t Channel::getValueBlue() { return value[2]; }

uint8_t Channel::getValueColorBrightness() { return value[1]; }

uint8_t Channel::getValueBrightness() { return value[0]; }

uint8_t Channel::getValueClosingPercentage() const {
  return value[0] >= 0 ? value[0] : 0;
}

bool Channel::getValueIsCalibrating() const { return value[0] == -1; }

uint8_t Channel::getValueTilt() const { return value[1] >= 0 ? value[1] : 0; }
...
bool Channel::isSleepingEnabled() { return validityTimeSec > 0; }
...
bool Channel::isBatteryPowered() const { return batteryPowered == 1; }
...
void Channel::setBatteryPowered(bool value) {
  ...
  if (newValue != batteryPowered) {
    SUPLA_LOG_DEBUG("Channel[%d] battery powered changed to %d", channelNumber,
                    newValue);
...
void Channel::setBatteryLevel(int level) {
  if (level >= 0 && level <= 100) {
    if (level != batteryLevel) {
      SUPLA_LOG_DEBUG("Channel[%d] battery level changed to %d", channelNumber,
                      level);
      batteryLevel = level;
      setSendStateInfo();
    }
...
bool Channel::isHvacFlagHeating() { return isHvacFlagHeating(getValueHvac()); }

bool Channel::isHvacFlagCooling() { return isHvacFlagCooling(getValueHvac()); }
...
enum Supla::HvacCoolSubfunctionFlag
Channel::getHvacFlagCoolSubfunction(THVACValue *hvacValue) {
  if (hvacValue != nullptr) {
...
uint8_t Channel::getHvacMode() const { return hvacValue.Mode; }
...
const char *Channel::getInitialCaption() const { return initialCaption; }

bool Channel::isInitialCaptionSet() const { return initialCaption != nullptr; }

void Channel::setDefaultIcon(uint8_t iconId) { defaultIcon = iconId; }

uint8_t Channel::getDefaultIcon() const { return defaultIcon; }
...
void Channel::fillDeviceChannelStruct(
  ...
  SUPLA_LOG_VERBOSE(
      ...
      getChannelNumber(), getChannelType(), getFuncList(), getDefaultFunction(),
      getFlags(),
      state == 0   ? "online"
      : state == 1 ? "offline"
                   : "online (not available)",
      validityTimeSec, getDefaultIcon(), static_cast<uint8_t>(value[0]),
      static_cast<uint8_t>(value[1]), static_cast<uint8_t>(value[2]),
      static_cast<uint8_t>(value[3]), static_cast<uint8_t>(value[4]),
      static_cast<uint8_t>(value[5]), static_cast<uint8_t>(value[6]),
...
void Channel::fillDeviceChannelStruct(
  ...
  SUPLA_LOG_VERBOSE(
      ...
      "[%02x %02x %02x %02x %02x %02x %02x %02x]",
      getChannelNumber(), getSubDeviceId(), getChannelType(), getFuncList(),
      getDefaultFunction(), getFlags(),
...
int8_t *Channel::getValuePtr() { return value; }
...
uint8_t Channel::getSubDeviceId() const { return subDeviceId; }
...
void Channel::setContainerFillValue(int8_t fillLevel) {
  ...
  runAction(ON_CHANGE);
  setSendValue();
  SUPLA_LOG_DEBUG("Container(%d) value changed to %d", channelNumber,
                  fillLevel);
}
...
bool Channel::isValveOpen() const { return getValveOpenState() > 0; }
