...
static void mqttEventHandler(void *handler_args, esp_event_base_t base,
  ...
  SUPLA_LOG_DEBUG(
      " *** MQTT Event dispatched from event loop base=%s, eventId=%d", base,
      eventId);
  ...
  case MQTT_EVENT_CONNECTED:
    SUPLA_LOG_INFO("MQTT_EVENT_CONNECTED");
  ...
  case MQTT_EVENT_DISCONNECTED:
    SUPLA_LOG_INFO("MQTT_EVENT_DISCONNECTED");
    espMqtt->setConnecting();
    break;
  ...
  case MQTT_EVENT_ERROR:
    SUPLA_LOG_INFO("MQTT_EVENT_ERROR");
    ...
    if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT) {
      ...
      SUPLA_LOG_DEBUG("Last captured errno : %d (%s)",
...
Supla::Protocol::EspMqtt::EspMqtt(SuplaDeviceClass *sdc)
  ...
  // SUPLA_LOG_DEBUG("   Creating EspMqtt");
  mutex = Supla::Mutex::Create();
  espMqtt = this;
}

Supla::Protocol::EspMqtt::~EspMqtt() { espMqtt = nullptr; }
...
void Supla::Protocol::EspMqtt::onInit() {
  ...
  esp_mqtt_client_register_event(client, MQTT_EVENT_ANY, mqttEventHandler,
                                 nullptr);
